...
class SaveResults:
    ...
    def _save_pointcloud(self, event_data: EventData) -> str:
        if event_data.pointcloud is None:
            return ''
        points = parse_point_cloud(
            event_data.pointcloud.point_core, event_data.pointcloud.point_supplement)
        print(
            f'get pointcloud: {len(points)}, points_size:{event_data.pointcloud.point_size}')
        ...
        timestamp_str = datetime.fromtimestamp(
            event_data.timestamp_ms / 1000).strftime("%Y%m%d_%H%M%S_%f")
    ...
    def _save_image(self, image_data: ImageData) -> str:
        ...
        timestamp_str = datetime.fromtimestamp(
            image_data.timestamp_ms / 1000).strftime("%Y%m%d_%H%M%S_%f")
    ...
    def save_to_excel(self):
        ...
        with pd.ExcelWriter(self.excel_file, engine='openpyxl') as writer:
            ...
            if pointcloud_col_index:
                ...
                for row_idx in range(2, worksheet.max_row + 1):
                    cell = worksheet.cell(
                        row=row_idx, column=pointcloud_col_index)
...
if __name__ == "__main__":
    ...
    cv2.putText(test_image, "Test Image", (10, 50),
