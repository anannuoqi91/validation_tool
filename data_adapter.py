...
def handle_event_region_attr(self, event_region_attr):
...
class RtspCameraSource(CameraSource):
    ...
    def __init__(self, rtsp_url: str):
        ...
        self.image_queue = queue.Queue(maxsize=10)  # Limit queue size to 10 frames
    ...
    def release(self):
        ...
        if hasattr(self, 'capture_thread') and self.capture_thread.is_alive():
            self.capture_thread.join(timeout=1.0)  # Add timeout to avoid blocking
...
class ChannelEventSource(EventSource):
    ...
    def __init__(self, channel_name: str = None,  pointcloud_channel_name: str = None, event_type: int = EventRegionAttribute.FLOW_EVENT):
        ...
        self.event_reader = self.cyber_node.create_reader(self.channel_name, BaseEvents, self._event_callback)
        
        if self.pointcloud_channel_name is not None:
            self.pointcloud_reader = self.cyber_node.create_reader(self.pointcloud_channel_name, PointCloud2, self._pointcloud_callback)
    ...
    def _event_callback(self, msg: BaseEvents):
        ...
        for base_event in msg.base_events:
            if base_event.event_region_attr != self.event_type:
                continue
            handle = handle_event_region_attr(base_event, base_event.event_region_attr)  
            ...
            if len(event.common_event.boxes) > 0:
                ...
                logger.error(f"Event {event.common_event.event_id} has no boxes")
        ...
        for channel_name, message, timestamp in self.record_reader.read_messages():
            ...
            if self.camera_channel == channel_name or \
                ...
                if camera_frame.data_format == DataFormat.OPENCV:
                    ...
                    image_data.image = np.frombuffer(camera_frame.data, dtype=np.uint8)
                    image_data.image = image_data.image.reshape((camera_frame.rows, camera_frame.cols, camera_frame.channels))
                else:
                    logger.error(f"Unsupported data format: {camera_frame.data_format}")
...
class DataAdapter:
    ...
    def set_online_mode(self, rtsp_url: str = None, event_channel: str = None, pointcloud_channel: str = None, event_type: int = EventRegionAttribute.FLOW_EVENT):
        ...
        if rtsp_url is None and event_channel is None:
            logger.error("RTSP URL and event channel cannot be None at the same time")
            return
        if rtsp_url is not None:
            self.camera_source = RtspCameraSource(rtsp_url)
        if event_channel is not None:
            if self.event_source is None:
                self.event_source = ChannelEventSource(event_channel, pointcloud_channel, event_type)
            else:
                self.event_source.set_channel_name(event_channel, pointcloud_channel)
    ...
    def set_offline_mode(self, record_path: str, camera_channel: str = None, event_channel: str = None, event_type: int = EventRegionAttribute.FLOW_EVENT, fps: int = None):
        ...
        # self._clear_sources()
        self.stop()
        self.record_source = RecordSource(record_path, camera_channel=camera_channel, event_channel=event_channel, event_type=event_type, fps=fps)
    ...
    def run(self, sync: bool = False):
        ...
        if self.mode is None:
            logger.error("Mode not set. Please call set_online_mode or set_offline_mode first.")
            return
        ...
        if sync:
            ...
            self.processing_thread = threading.Thread(target=self._run_with_mode)
...
if __name__ == "__main__":
    ...
    def event_callback(event_data):
        print(f"Event in region {event_data.region_name} at {event_data.timestamp_ms} ms")
    ...
    try:
        ...
        adapter.set_online_mode(event_channel="omnisense/event/172.30.0.3_1/events")
